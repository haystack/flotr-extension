...
Exhibit.ScatterPlotView.prototype._reconstruct = function() {
    ...
    prepareData = function() {
        var scaleX, scaleY, unscaleX, unscaleY, currentSet, xAxisMin, yAxisMin, xAxisMax, yAxisMax, i, xys, color, colorKeys, size, sizeKeys;
        ...
        currentSet.visit(function(itemID) {
            ...
            if (xys.length > 0) {
                ...
                for (var i = 0; i < xys.length; i++) {
                    ...
                    dataToPlot[color].push([xy.scaledX, xy.scaledY, size])
...
Exhibit.ScatterPlotView.prototype._clickHandler = function(flotr, container, xyToData) {
    var self, hitEvt_x, hitEvt_y, hitEvt_absX, hitEvt_absY, popupPresent;
    self = this;
    popupPresent = false;
    ...
    Exhibit.jQuery("body").click( function(e) {
        ...
        if (!popupPresent) {
            ...
            disX = Math.abs(e.pageX - hitEvt_absX);
            disY = Math.abs(e.pageY - hitEvt_absY);
            distance = Math.sqrt(disX * disX + disY * disY);    

            if (distance < 10) {
                key = hitEvt_x + "," + hitEvt_y;
...
Exhibit.ScatterPlotView.prototype._createFlotrScatter = function(flotr, container, dataToPlot, xyToData, legendLabels) {
    var self, settings, dataList, i, trackFn, x, y, key, id, graph;
    self = this;
    settings = this._settings;
    dataList = [];
